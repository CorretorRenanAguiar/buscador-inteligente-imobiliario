...
from database import salvar_lead_supabase
...
def criar_relatorio(sessao, whatsapp, qualificacao):
    ...
    intencao = qualificacao.get(
        "intencao_compra",
        "NÃO CLASSIFICADA",
    )

    maturidade = qualificacao.get(
        "maturidade",
        "NÃO CLASSIFICADA",
    )

    prioridade = qualificacao.get(
        "prioridade",
        "NÃO CLASSIFICADA",
    )
...
async def processar_chatbot(mensagem, session_id):
    ...
    if etapa in ["whatsapp", "whatsapp_permuta"]:
        ...
        # Motor híbrido:
        # filtros determinísticos + score + K-Means + prioridade.
        qualificacao = qualificar_sessao_chatbot(sessao)

        # Persistência do lead e da qualificação no Supabase.
        ...
        if not persistencia.get("sucesso"):
            logger.error(
                "Lead qualificado, mas não foi salvo no Supabase: %s",
                persistencia.get("erro"),
            )
            resposta["aviso_banco"] = (
                "O atendimento foi concluído, mas houve uma falha ao "
